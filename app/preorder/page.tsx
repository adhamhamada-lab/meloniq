...
function PreorderContent() {
  ...
  async function validateDiscount() {
    ...
    setValidating(false);
    if (data.valid) { setDiscountStatus("valid"); setDiscountInfo({ type: data.type, value: data.value }); }
    else { setDiscountStatus("invalid"); setDiscountInfo(null); }
  }
      ...
      <section className="px-6 md:px-20 pt-10 pb-24">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr] items-start gap-10 md:gap-20 max-w-[1400px] mx-auto">
          ...
          <div className="lg:sticky lg:top-32">
            <p className="tracking-[0.35em] text-[#66705D] text-sm uppercase">{t.checkoutLabel}</p>
            ...
            <div className="mt-8 flex flex-col gap-3">
              <div className="flex items-start gap-3">
                ...
                <p className="text-[#66705D]">{t.orderBullet1}</p>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-[#55614A] text-lg mt-1">✦</span>
                <p className="text-[#66705D]">{t.orderBullet2}</p>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-[#55614A] text-lg mt-1">✦</span>
                <p className="text-[#66705D]">{t.orderBullet3}</p>
              </div>
            </div>
          </div>
          ...
          {done ? (
            <div className="bg-[#D7DCCB] rounded-[40px] p-10 text-center shadow-2xl mt-10">
              ...
              <h2 className="text-[40px] text-[#55614A]">{t.orderReceivedTitle}</h2>
              <p className="mt-4 text-[#66705D]">{t.orderReceivedDesc}</p>
            ...
            <form onSubmit={send} className="bg-[#D7DCCB] rounded-[30px] md:rounded-[60px] p-6 md:p-14 shadow-2xl w-full max-w-[700px] mx-auto flex flex-col gap-6 mt-10">
              ...
              <input required name="name" placeholder={t.name} className={inputStyle} />

              <div className="flex flex-col gap-4">
                <p className="text-[#66705D] tracking-[0.15em] uppercase text-sm px-2">{t.selectProduct}</p>
                {items.map((item, index) => (
                  <div key={index} className="flex gap-2 items-center w-full">
                    ...
                    />
                    {items.length > 1 && (
                      <button type="button" onClick={() => removeItem(index)} className="w-9 h-9 rounded-full bg-white text-[#55614A] hover:bg-[#55614A] hover:text-white duration-300 flex items-center justify-center shrink-0">
              ...
              <input required name="contact" placeholder={t.phone} className={inputStyle} />
              <textarea required name="address" placeholder={t.address} rows={4} className="bg-white text-[#55614A] placeholder:text-[#7C8572] rounded-[32px] px-7 py-5 outline-none border border-transparent focus:border-[#55614A] resize-none duration-300 text-lg" />

              {/* DISCOUNT CODE */}
              <div className="flex flex-col gap-2">
                <p className="text-[#66705D] tracking-[0.15em] uppercase text-sm px-2">{t.discountCode}</p>
                <div className="flex flex-col sm:flex-row gap-3">
                  <input
                    type="text" value={discountCode}
                    onChange={(e) => { setDiscountCode(e.target.value); setDiscountStatus("idle"); setDiscountInfo(null); }}
                  ...
                  />
                  <button type="button" onClick={validateDiscount} disabled={validating || !discountCode.trim()} className="px-6 py-4 rounded-full border border-[#55614A] text-[#55614A] text-sm uppercase tracking-[0.1em] hover:bg-[#55614A] hover:text-white duration-300 disabled:opacity-50">
                    {validating ? "..." : t.apply}
                  </button>
                </div>
                {discountStatus === "valid" && discountInfo && (
                  <p className="text-[#55614A] text-sm px-2">✓ {t.discountCode} — {discountInfo.value}{discountInfo.type === "percentage" ? "%" : " EGP"}</p>
                )}
              ...
              {validItems.length > 0 && (
                <div className="bg-white rounded-[24px] p-6 flex flex-col gap-4">
                  <p className="text-[#66705D] tracking-[0.15em] uppercase text-sm">{t.yourOrderTitle}</p>
                  {validItems.map((item, i) => {
                    const product = PRODUCTS.find((p) => p.name === item.product);
                    return (
                      <div key={i} className="flex items-center gap-4">
                        {product?.image && <Image src={product.image} alt={item.product} width={60} height={60} className="w-[60px] h-[60px] rounded-[12px] object-cover shrink-0" />}
                  ...
                  <div className="border-t border-[#D7DCCB] pt-3 flex justify-between items-center">
                    <p className="text-[#55614A] font-medium">{t.totalLabel}</p>
              ...
              <button disabled={loading} className="mt-2 bg-[#55614A] text-white rounded-full py-6 text-xl uppercase tracking-[0.12em] hover:scale-[1.02] hover:opacity-95 duration-300 disabled:opacity-70">
